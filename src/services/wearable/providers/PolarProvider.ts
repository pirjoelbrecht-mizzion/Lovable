...
export class PolarProvider implements ProviderInterface {
  name = 'polar' as const;
  ...
  async fetchMetrics(dateISO: string): Promise<WearableMetric | null> {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/polar-fetch`,
        {
          method: 'POST',
          headers: {
      ...
      return {
        ...
        recoveryTime: data.nightlyRecharge
      };
    } catch (error) {
  ...
  async refreshToken(): Promise<boolean> {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/polar-refresh-token`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
